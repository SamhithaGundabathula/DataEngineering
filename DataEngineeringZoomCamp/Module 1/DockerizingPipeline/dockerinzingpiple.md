...
so how basically a docker works is. You create a file -> build an image -> run the image -> use container.
...
```
if your docker filename is not default then:

```bash
docker build -f filename -t test:pandas .
```

building an image where test -> name of the image and pandas -> tag
...
What about uv? Let's use it instead of using pip: I have'nt done this
