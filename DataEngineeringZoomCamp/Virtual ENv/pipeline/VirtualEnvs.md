...
here pyarrow is used to manage parquet file
...
pip install uv
```
uv creates and manages a project-specific Python environment for this repository.
This environment has:

its own Python version (as defined in .python-version)

its own dependencies (as defined in pyproject.toml)

When we run Python using uv run, the script is executed using this isolated environment, not the system Python.
...
The interesting way to verify is, try running the pipeline.py file (python filename - uses system python) it may throw error because pandas or pyarrow is not installed in your system. But runs using UV since it is present in your project folder.